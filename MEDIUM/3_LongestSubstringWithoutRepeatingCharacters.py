...
class Solution:
    def lengthOfLongestSubstring(self, s: str) -> int:
        ...
        for r in range(len(s)):
            while s[r] in charSet: # remove left pointer values into s[r] is out of the charSet
                charSet.remove(s[l])
                l += 1
            charSet.add(s[r])
            res = max(res, r - l + 1)
...
once a duplicate character is found in our set, we remove the character at the left pointer until the duplicate character (at r) is gone. 
we continuously update the left pointer to shrink the window as this is happening.
